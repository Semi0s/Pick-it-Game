...
test("Easy Bracket and Strategy Mode conflict messaging is explicit", () => {
  assert.match(getModePreviewConflictMessage("easy_bracket", "strategy_mode") ?? "", /active Easy Bracket/i);
  assert.match(getModePreviewConflictMessage("strategy_mode", "easy_bracket") ?? "", /active Strategy Mode/i);
});
