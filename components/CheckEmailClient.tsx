...
export function CheckEmailClient({ language }: { language?: string | null }) {
  ...
  return (
    <div className="rounded-[1.5rem] border border-white/65 bg-white/95 p-5 text-center shadow-2xl shadow-black/25">
      ...
      {email ? (
        <p className="mt-2 text-sm font-bold text-red-700">{maskConfirmationEmail(email)}</p>
      ) : (
        <p className="mt-2 text-sm font-semibold text-red-700">{t(uiLanguage, "auth.checkEmailGenericAddress")}</p>
      )}
