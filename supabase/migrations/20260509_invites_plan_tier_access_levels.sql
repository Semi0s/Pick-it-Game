...
alter table public.invites
  drop constraint if exists invites_plan_tier_check;

alter table public.invites
  add constraint invites_plan_tier_check
  check (plan_tier in ('player', 'captain', 'manager', 'director', 'managing_director'));

update public.invites
set plan_tier = 'player'
where coalesce(nullif(trim(plan_tier), ''), 'player') <> plan_tier;
