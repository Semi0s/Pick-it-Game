...
create table public.user_report_actions (
  id uuid primary key default gen_random_uuid(),
...
create index user_reports_target_idx
  on public.user_reports (target_type, target_id);

create index user_reports_group_status_created_idx
...
create index user_report_actions_report_created_idx
  on public.user_report_actions (report_id, created_at desc);

create index user_report_actions_actor_created_idx
...
on public.user_report_actions for select
to authenticated
using (public.is_super_admin(auth.uid()));
